...
<body>
    <div class="modal fade show" id="popup442073" tabindex="-1" aria-labelledby="popupLabel442073" remove-close="true"
        ...
        <div class="modal-dialog modal-lg modal-center">
            <div class="modal-content bg-add-aeroLine">
                <div class="modal-header">
                    <div class="modal-title">
                        <h5 style="width: 100px" id="popupLabel442073">Запланировать работу</h5>
                        <div class="message">
                            @if (session('text'))
                ...
                <form action="{{ route('addAeroLinePost', ['aeroLine_id' => $aeroLine_id]) }}" method="POST">
                    @csrf

                    {{-- <div class="modal-body" style="max-height:75vh; overflow:auto; overflow-x:hidden;">
                            <div class="id_name">
                                <div class="token col-sm-2 pb-2">
                                    <label style="padding-top: 5px;" class="form-label">id ВЛ</label>
                                    <input type="number" id="aeroLine_id" value="{{ $aeroLine_id }}" placeholder=""
                                        name="aeroLine_id" class="form-control form-control-default" required=""
                                        autocomplete="off" data-mask="phone-1" data-init-mask="true">
                                </div>
                                <div class="token col-sm-5 pb-2">
                                    <label style="padding-top: 5px;" class="form-label">Наименование ВЛ</label>
                                    <input type="text" id="aeroLine_name" value="{{ $aeroLineName }}"
                    ...
                    <div class="modal-body">
                        <div class="id_name-aeroLine">
                            <div class="token col-sm-12 pb-2">
                                <label style="padding-top: 5px;" class="form-label">Наименование ВЛ</label>
                                <input type="text" id="aeroLine_name" value="{{ $aeroLineName }}" placeholder=""
    ...
    <script>
        $(document).ready(function() {
            $('#aeroLine_id').on('input', function() {
                let id = $(this).val();
                if (id) {
                    $.ajax({
                        url: `{{ url('/get-aeroLine-name') }}/${id}`,
                        type: 'GET',
                        success: function(response) {
                            $('#aeroLine_name').val(response.name);
                        }
                    });
                } else {
                    $('#aeroLine_name').val('');
    ...
    <script>
        ...
        const input = document.querySelector('#aeroLine_name1');
