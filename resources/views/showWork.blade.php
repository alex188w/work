...
@section('content')
                ...
                <tr>
                    ...
                    <td width="100">
                        <button class="{{ $work->is_done === 1 ? 'completed' : 'bg-info btn-info not_completed' }}">
                            <a class="link"
                                href="{{ route('edit', ['ustanovka_id' => $work->ustanovka_id, 'work_id' => $work->id]) }}">{{ $work->is_done === 1 ? 'Просмотреть' : 'Выполнить' }}</a>
                        </button>
                    </td>
                </tr>
            @endforeach
    ...
    <button class="add"><a class="link"
            href="{{ route('addAction', ['ustanovka_id' => $ustanovka->id]) }}">Запланировать работу</a></button>
...
@section('sidebar')
    <div class="sidebar_show">
        ...
        <div class="button-chart">
            <button class="btn btn-primary open-chart" data-id="{{ $ustanovka->id }}" data-type="I"
                data-bs-target="#chartModalI">
                График I(t)
            </button>
            <button class="btn btn-secondary open-chart" data-id="{{ $ustanovka->id }}" data-type="U"
                data-bs-target="#chartModalU">
                График U(t)
            </button>

            <button class="btn btn-success open-chart" data-id="{{ $ustanovka->id }}" data-type="R"
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function renderChart(data, type, canvasId) {
            ...
            if (type === 'I') {
                values = data.map(item => parseFloat(item.I));
                label = 'Ток (A)';
                color = 'blue';
            } else if (type === 'U') {
                values = data.map(item => parseFloat(item.U));
                label = 'Напряжение (V)';
                color = 'red';
            } else if (type === 'R') {
                values = data.map(item => parseFloat(item.I) !== 0 ? parseFloat(item.U) / parseFloat(item.I) :
                    null);
